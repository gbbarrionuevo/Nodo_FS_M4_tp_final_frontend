...
const RolesAdminCreate = () => {
  const navigate = useNavigate();
  const { create } = useRolesCrud();
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await alert("Estás seguro de crear el rol?", {
      icon: "warning",
    ...
    try {
      ...
      const msg = err.response?.data?.errors?.[0]?.message || err.response?.data?.error || "Error al crear el rol";
      notify(msg, "error");
    }
  };
              ...
              <div className="col-span-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {permissionsData.map((p) => (
                    <label key={p._id} className="flex items-center gap-2 bg-gray-100 dark:bg-gray-700 p-2 rounded">
                      <input
                        type="checkbox"
...
export default RolesAdminCreate;