...
const UsersAdminEdit = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { loading, show, update } = useUsersCrud();
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await alert("Estás seguro de editar el usuario?", {
      icon: "warning",
    ...
    try {
      ...
      notify("Usuario editado correctamente", "success");
      navigate("/administration/users");
    } catch (err) {
      const msg = err.response?.data?.errors?.[0]?.message || err.response?.data?.error || "Error al editar el usuario";
      notify(msg, "error");
    }
  };
        ...
        <div className="flex items-start justify-center p-4">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-1">
                <label className="block mb-1 text-sm font-medium">Nueva contraseña (opcional)</label>
                <Input
                  type="password"
                  name="password"
                  value={form.password}
